...
<script>
// import { validatePassword, validateTel, validateErrorText } from '@/utils/validate'
...
export default {
  ...
  data () {
    ...
    //   if (!value) {
    //     callback(new Error(validateErrorText('手机号')))
    //   }
    ...
    //     callback(new Error('密码必须包含英文和数字且长度大于等于6位'))
    //   } else {
    //     callback()
    //   }
    // }
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch('user/login', this.loginForm)
            ...
            .catch(() => {
              // this.$message.error('您的账号暂时无法使用，请稍后再试试吧~')
              this.loading = false
            })
        } else {
